...
func handleUnifiedAdmin(ctx context.Context, req *mcp.CallToolRequest, input UnifiedAdminInput) (*mcp.CallToolResult, interface{}, error) {
	action := strings.TrimSpace(strings.ToLower(input.Action))
	if action == "" {
		return nil, nil, errors.New("action is required (consolidate|cleanup|orgs|export|import|plan_create|plan_status|plan_list|plan_apply|plan_cancel|analyze)")
	}
	...
	default:
		return nil, nil, fmt.Errorf("invalid action '%s'. Valid actions: consolidate, cleanup, orgs, export, import, plan_create, plan_status, plan_list, plan_apply, plan_cancel, analyze", action)
	}
}
